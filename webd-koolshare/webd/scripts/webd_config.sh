...
fun_nat_start(){
    if [ "${webd_enable}"x = "1"x ] ;then
	    [ ! -L "/koolshare/init.d/S39webd.sh" ] && ln -sf /koolshare/scripts/webd_config.sh /koolshare/init.d/S39webd.sh
    fi
}
# 定时任务
fun_crontab(){
    if [ "${webd_enable}" != "1" ] || [ "${webd_cron_time}"x = "0"x ];then
     ...
     if [ "${webd_cron_hour_min}" == "min" ] && [ "${webd_cron_time}"x != "0"x ] ; then
        if [ "${webd_cron_type}" == "watch" ]; then
        	cru a webd_monitor "*/"${webd_cron_time}" * * * * /bin/sh /koolshare/scripts/webd_config.sh watch"
        elif [ "${webd_cron_type}" == "start" ]; then
            cru a webd_monitor "*/"${webd_cron_time}" * * * * /bin/sh /koolshare/scripts/webd_config.sh restart"
    	fi
    elif [ "${webd_cron_hour_min}" == "hour" ] && [ "${webd_cron_time}"x != "0"x ] ; then
        if [ "${webd_cron_type}" == "watch" ]; then
            cru a webd_monitor "0 */"${webd_cron_time}" * * * /bin/sh /koolshare/scripts/webd_config.sh watch"
        elif [ "${webd_cron_type}" == "start" ]; then
            cru a webd_monitor "0 */"${webd_cron_time}" * * * /bin/sh /koolshare/scripts/webd_config.sh restart"
        fi
    fi
}
